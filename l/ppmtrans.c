 ...
 *****************************************************************/
static void execution(FILE *fp, A2Methods_T methods, int rotation, 
                char *flip_type, CPUTime_T timer, char *time_file_name, double time_used)
{
        ...
        rotation(flip_type, rotation);

        time_used = CPUTime_Stop(timer); /*stop timer*/

        write_the_timing(char *time_file_name, double time_used, int width, int height);
...
static void rotation(char *flip_type, int rotation)
{
        if (flip_type == NULL) {
                if (rotation == 90) {
...
static void write_the_timing(char *time_file_name, double time_used, int width, int height)
{
...
int main(int argc, char *argv[])
{
        ...
        if (argc == 1) { /* nothing provided */
                fp = stdin;
        } else if (rotation != 0  && flip_type != NULL) { /* can't rotate and 
                                                        flip */
                fprintf(stderr, "Too many arguments\n"); 
                return EXIT_FAILURE;
        } else {
        ...
        if (rotation != 0) {
                execution(fp, methods, rotation, NULL, timer, time_file_name, time_used);
        } else {
                execution(fp, methods, 0, flip_type, timer, time_file_name, time_used);
        }
